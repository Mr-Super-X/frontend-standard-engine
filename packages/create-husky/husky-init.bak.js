...
const projectDirectory = cwd(), // 项目目录
   ...
   * husky命令枚举 
   * 
   * npx husky-init会做以下事情
   *    1. npm i husky -D
   *    2. 在package.json的scripts脚本中添加"prepare": "husky install"
   *    3. 执行husky install在.git目录中创建git hook
   *    4. 在.husky目录中创建pre-commit脚本文件，并输入npm test
   */
  huskyCommandMap = {
    npm: "npx husky-init && npm install && npm install --save-dev ",
    yarn: "npx husky-init && yarn && yarn add --dev ",
...
async function init() {
  ...
  const packages = commitlint
    ? `${preCommitPackages} ${commitMsgPackages}`
    : preCommitPackages;
  // 判断用户是否选择commitlint，执行不同的操作
  const createHookCommand = commitlint
    ? `${createCommitHook} && ${createMsgHook}`
    : createCommitHook;
  ...
  const lintStagedContent = `module.exports = ${JSON.stringify(
    getLintStagedOption(selectLint || pakHasLint)
  )}`;
  // 创建安装进度
